...
function updateURL(bookId, chapterId, version, verseId = null) {
    let newUrl = `${window.location.pathname}?bookId=${encodeURIComponent(bookId)}&chapterId=${encodeURIComponent(chapterId)}&versionId=${encodeURIComponent(version.toLowerCase())}`;
...
function reloadChapterContent(bookId, chapterId, version, verseId = null) {
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            document.getElementById('iqbible-chapter-results').innerHTML = response.chapterContent;
            document.getElementById('fetch-books-header').textContent = response.bookName + ' ' + parseInt(chapterId, 10);
            document.getElementById('fetch-books-header-version').innerText = ' (' + version.toUpperCase() + ')';
            
            // Set text direction based on version
            const mainContent = document.getElementById('iqbible-bible-content-wrapper');
    ...
    xhr.send('action=iq_bible_chapter_ajax_handler&bookId=' + encodeURIComponent(bookId) +
             '&chapterId=' + encodeURIComponent(chapterId) +
             '&versionId=' + encodeURIComponent(version.toLowerCase()) +
...
function checkAudioNarration(bookId, chapterId, version) {
    ...
    xhrAudio.send('action=iq_bible_audio_check&bookId=' + encodeURIComponent(bookId) + 
                  '&chapterId=' + encodeURIComponent(chapterId) + 
                  '&versionId=' + encodeURIComponent(version.toLowerCase()));
}
...
function openTab(tabId) {
    ...
    var activeButton = document.querySelector('.iqbible-tab-button[onclick="openTab(\'' + tabId + '\')"]');
    if (activeButton) {
...
// ------------------
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('iqbible-reading-plan-form').addEventListener('submit', function(e) {
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                contentDiv.querySelectorAll('.reading-plan-link').forEach(function(link) {
...
reloadChapterContent(currentBookId, currentChapterId, versionId); // Load the chapter content
                ...
                document.getElementById('iqbible-reading-plan-content').innerHTML = '<p>An error occurred while fetching the reading plan.</p>';
            }
        };
        ...
        xhr.send(
            ...
            '&iqbible-planName=' + encodeURIComponent(planName)  // Include the plan name
        );
    });

    // Show/Hide custom days input based on selection
    document.getElementById('iqbible-days').addEventListener('change', function() {
...
// --------------
    ...
    document.getElementById('iqbible-search-form').addEventListener('submit', function(e) {
        e.preventDefault();
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                // Display the results
                document.getElementById('iqbible-search-results').innerHTML = xhr.responseText;
        ...
        xhr.send('action=iq_bible_search&query=' + encodeURIComponent(query) + '&versionId=' + encodeURIComponent(versionId));
...
// --------------
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('iqbible-dictionary-form').addEventListener('submit', function(e) {
        e.preventDefault();
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                document.getElementById('iqbible-definition-results').innerHTML = xhr.responseText;
            } else {
                document.getElementById('iqbible-definition-results').innerHTML = '<p>An error occurred during the definition retrieval: </p>' + xhr.status ;
            }
        };
        xhr.send('action=iq_bible_define&iqbible-definition-query=' + encodeURIComponent(query));
    });
});
...
// --------------
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('iqbible-strongs-form').addEventListener('submit', function(e) {
        e.preventDefault();
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                document.getElementById('iqbible-strongs-results').innerHTML = xhr.responseText;
            } else {
                document.getElementById('iqbible-strongs-results').innerHTML = '<p>An error occurred during the concordance retrieval: </p>' + xhr.status;
            }
        };
        // Use the correct action name for Strong's
        xhr.send('action=iq_bible_strongs_ajax_handler&lexicon=' + encodeURIComponent(lexicon) + '&id=' + encodeURIComponent(id)); // Send lexicon and id
    });
});
...
function showCrossReferences(verseId) {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            crossRefsContainer.querySelectorAll('.cross-reference-link').forEach(function(link) {
    ...
    xhr.send('action=iq_bible_get_cross_references&verseId=' + encodeURIComponent(verseId));
}
...
function showOriginalText(verseId) {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            alert('An error occurred while retrieving the original text: ' + xhr.status);
        }
    };
    xhr.send('action=iq_bible_get_original_text&verseId=' + encodeURIComponent(verseId));
}
...
// Copy verse
// ------------------
function copyVerse(verseId, bookName, chapterNumber, versionId, siteName, language) {
    // Find the verse container
    var verseElement = document.getElementById('verse-' + verseId);
...
document.querySelectorAll('.story-link').forEach(function(storyLink) {
    storyLink.addEventListener('click', function(e) {
        ...
        reloadChapterContent(currentBookId, currentChapterId, versionId, 'story-'+verseId);
...
function getChapterCount(bookId, callback) {
    ...
    xhrChapterCount.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');

    xhrChapterCount.onload = function () {
        if (xhrChapterCount.status === 200) {
            var response = JSON.parse(xhrChapterCount.responseText);
            if (response.chapterCount) {  // Check for chapterCount directly
    ...
    xhrChapterCount.send('action=iq_bible_chapter_count_ajax_handler&bookId=' + bookId);
}
...
document.querySelectorAll('.iqbible-topic-item').forEach(function(topicItem) {
    topicItem.addEventListener('click', function() {
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                if (existingResults && existingResults.classList.contains('topic-results')) {
                    existingResults.remove();
                }

                // Insert the new results after the clicked topic
                topicElement.parentNode.insertBefore(topicResultsDiv, topicElement.nextSibling);

                // Add click handlers to the newly created verse links
                initializeTopicVerseLinks(topicResultsDiv);
            } 
        };
        xhr.send('action=iq_bible_topics_ajax_handler&topic=' + encodeURIComponent(topic));
    });
});
...
function initializeTopicVerseLinks(container) {
    ...
    reloadChapterContent(currentBookId, currentChapterId, selectedVersionName,verseId);
    


    // Open Book Dialog when the "Fetch Books" header is clicked
    // -----------------------------------------------------------
    document.getElementById('fetch-books-header').addEventListener('click', function () {
        var xhr = new XMLHttpRequest();
        xhr.open('POST', iqbible_ajax.ajaxurl, true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        ...
        xhr.send('action=iq_bible_books_ajax_handler');
    });
...
button.addEventListener('click', function () {
    ...
    document.getElementById('book-dialog').close();
                    ...
                    var bookItem = document.querySelector('.book-item[data-book-id="' + bookId + '"]');
                    if (bookItem) {
            ...
            xhr.send('action=iq_bible_chapter_count_ajax_handler&bookId=' + encodeURIComponent(bookId) + '&bookCategory=' + encodeURIComponent(bookCategory));
        }
    });
...
document.getElementById('fetch-books-header-version').addEventListener('click', function () {
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            var groupedVersions = {};
            response.forEach(function (version) {
                var language = version.language.charAt(0).toUpperCase() + version.language.slice(1);
            ...
            for (var language in groupedVersions) {
                ...
                groupedVersions[language].forEach(function (version) {
                    ...
                    listItem.addEventListener('click', function () {
                        ...
                        selectedLanguage = this.getAttribute('data-version-language').toLowerCase();
...
function clearBooksSession(language) {
    ...
    xhr.send('action=clear_books_session&language=' + encodeURIComponent(language));
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (iqbible_ajax.isUserLoggedIn == '1') { // is logged in
            ...
            if (xhr.status === 200) {
                ...
                if (response.success) {
                    ...
                    response.data.forEach(function (note) {
                        ...
                        var noteTextDecoded = note.note_text.replace(/\\'/g, "'").replace(/\\"/g, '"').replace(/\\\\/g, "\\");
...
noteTextTitle = getFirstTenWords(noteTextDecoded);
                    ...
                    document.getElementById('iqbible-notes-list').innerHTML = '<p>No notes found.</p>';
                }
            } 
        };

        xhr.send('action=iq_bible_get_saved_notes');
    }
    ...
    function editNote() {
        ...
        var noteContent = this.closest('.note-item').querySelector('.note-content').innerHTML;
    ...
    function deleteNote() {
        ...
        if (confirm('Are you sure you want to delete this note?')) {
            ...
            xhr.send('action=iq_bible_delete_note&note_id=' + encodeURIComponent(noteId));
        }
    }
});
...
function showCommentary(verseId) {
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            var response = JSON.parse(xhr.responseText);
            var commentaryContent = response.commentary || 'No commentary available for this verse.';
    ...
    xhr.send('action=iq_bible_commentary_ajax_handler&verseId=' + encodeURIComponent(verseId));
}
...
function saveVerse(verseId) {
    ...
    var messageDiv = document.getElementById('verse-message-' + verseId);
        ...
        if (iqbible_ajax.isUserLoggedIn !== '1') {
            if (messageDiv) {
                messageDiv.innerHTML = '<span id="iqbible-error-text">You must be logged in to save verses</span>';
    ...
    xhr.onload = function () {
        ...
        if (xhr.status === 200) {
            ...
            if (response.success) {
                // Update message
                if (messageDiv) {
                    messageDiv.innerHTML = '<span id="iqbible-success-text">Verse saved</span>';
                    setTimeout(() => { messageDiv.textContent = ''; }, 3000);
                }
                ...
                // Update message
                if (messageDiv) {
                    messageDiv.innerHTML = '<span id="iqbible-error-text">Verse already saved</span>';
                    setTimeout(() => { messageDiv.textContent = ''; }, 3000);
                }
            }
        }
    ...
    var verseText = verseElement ? verseElement.querySelector('.copyable-text').textContent : '';

    // Send the verse text along with other data
    xhr.send(
        'action=iq_bible_save_verse' + 
        '&verseId=' + encodeURIComponent(verseId) + 
    ...
    // ------------------------------------------------------------------
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelector('button[onclick="openTab(\'profile\')"]').addEventListener('click', function() {
            if (iqbible_ajax.isUserLoggedIn == '1') { // is logged in
            loadSavedVerses();
            }
        });
...
function loadSavedVerses() {
    ...
    xhr.send('action=iq_bible_get_saved_verses');
}
        ...
        verseElement.innerHTML = `
            <div class="verse-content">
                <div class="verse-text">
                    ${verse.verseText} - ${verse.bookName}:${parseInt(verse.verseNumber, 10)} 
                    <span class="version-id">(${verse.versionId.toUpperCase()})</span>
                </div>
                <div class="saved-date"><small>Saved on ${formattedDate}</small></div>
                <button onclick="deleteVerse('${verse.verseId}')" class="delete-verse">Remove</button>
                <p></p>
            </div>
        `;
...
function deleteVerse(verseId) {
    if (!confirm('Are you sure you want to remove this verse from your saved verses?')) {
        return;
    }
        ...
        if (xhr.status === 200) {
            ...
            if (response.success) {
                ...
                if (window.savedVerses.length === 0) {
                    document.querySelector('.my-saved-verses').innerHTML = '<p>No saved verses.</p>';
                }
            } else {
    ...
    xhr.send('action=iq_bible_delete_saved_verse&verseId=' + encodeURIComponent(verseId));
}
...
function shareVerse(verseId) {
    ...
    var button = document.querySelector(`button[onclick*='shareVerse("${verseId}")']`);
    ...
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(verseUrl)
            .then(() => {
                var messageDiv = document.getElementById('verse-message-' + verseId);
                if (messageDiv) {
                    messageDiv.innerHTML = '<span id="iqbible-success-text">Link copied to clipboard</span>';
                    setTimeout(() => { messageDiv.textContent = ''; }, 3000);
                }
            })
            .catch((error) => {
